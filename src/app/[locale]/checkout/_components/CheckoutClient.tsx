...
const paymentMethods = [
  {id: 'mada', label: 'mada', icon: '/icons/pay-mada.svg', note: 'Saudi local payment option'},
  {id: 'paypal', label: 'PayPal', icon: '/icons/pay-paypal.svg', note: 'Fast online payments'},
  {id: 'applepay', label: 'Apple Pay', icon: '/icons/pay-applepay.svg', note: 'One tap on supported devices'},
  ...
  return (
    <>
      ...
      <motion.div
        ...
        className="absolute left-[-12%] top-[-8%] h-[145%] w-[58%] origin-top-left rotate-[-24deg] overflow-hidden rounded-[64px] bg-[linear-gradient(180deg,rgba(199,228,177,0.75)_0%,rgba(170,214,153,0.52)_15%,rgba(88,155,112,0.22)_42%,rgba(8,20,15,0.00)_86%)] shadow-[inset_0_0_80px_rgba(255,255,255,0.07)]"
      >
        <motion.div
          animate={{x: [0, 16, -12, 0]}}
...
export function CheckoutClient({
  ...
  const [selectedPayment, setSelectedPayment] = React.useState<string>('mada');
  ...
  const discount = React.useMemo(() => {
    ...
    if (code === 'FATE10') return Math.round(subtotal * 0.1);
  ...
  function applyCoupon() {
    ...
    if (code === 'FATE10' || code === 'WELCOME50') {
      setAppliedCoupon(code);
      setCouponNotice(code === 'FATE10' ? '10% discount applied.' : '$0.50 discount applied.');
    } else {
      setAppliedCoupon('');
  ...
  async function submitCheckout() {
    ...
    if (selectedPayment !== 'card') {
      setError(`${paymentLabel(selectedPayment)} UI is ready. Real activation comes with the payment provider setup.`);
      return;
    }
  ...
  return (
    <div className="relative min-h-screen overflow-hidden bg-[#040505] text-white">
      ...
      <div className="relative z-10 grid min-h-screen lg:grid-cols-[minmax(380px,42vw)_minmax(0,1fr)]">
        ...
        <div className="relative flex min-h-screen justify-center px-4 py-8 sm:px-6 lg:px-10">
          <div className="w-full max-w-[760px] pb-20 pt-4 lg:pt-8">
            <div className="flex items-center justify-between">
              ...
              <div className="text-right text-[16px] font-medium text-white/80">Premium Checkout</div>
            </div>

            <div className="mt-10">
              <h1 className="text-[30px] font-medium leading-tight tracking-[-0.03em] text-white sm:text-[42px]">
                Subscribe, and be part of tale premium community
              </h1>
              <p className="mt-3 text-lg text-white/62">Get access to all features and benefits</p>
            </div>

            <div className="mt-8 rounded-[24px] bg-[linear-gradient(180deg,rgba(14,16,19,0.94),rgba(10,11,14,0.92))] p-5 shadow-[0_35px_90px_rgba(0,0,0,0.42)]">
              {firstItem ? (
                <>
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex items-start gap-4">
                      <div className="flex h-14 w-14 items-center justify-center rounded-2xl bg-white/[0.06] shadow-[inset_0_0_0_1px_rgba(255,255,255,0.07)]">
                        <img src="/icons/apps.svg" alt="product" className="h-7 w-7 opacity-90" />
                      </div>
                      <div>
                        <div className="text-[26px] font-medium tracking-[-0.02em] text-white">{firstItem.name}</div>
                        <div className="mt-1 text-base text-white/58">{durationLabel(firstItem.periodMonths)}</div>
                      </div>
                    </div>
                    <div className="text-right text-[28px] font-semibold text-white">{formatUsdFromCents(lineTotal(firstItem))}</div>
                  </div>
                  ...
                  <div className="mt-7 space-y-3 border-t border-dashed border-violet-300/10 pt-5">
                    ...
                    <div className="flex items-center justify-between text-[24px] font-semibold text-white">
                      <span>Total:</span>
                  ...
                  <div className="mt-6 grid gap-3 sm:grid-cols-[1fr_88px]">
                    <input
                      ...
                      className="h-14 rounded-2xl bg-white/[0.06] px-5 text-[17px] text-white outline-none transition placeholder:text-white/30 shadow-[inset_0_0_0_1px_rgba(255,255,255,0.07)] focus:shadow-[inset_0_0_0_1px_rgba(167,139,250,0.45)]"
                    />
                    <button
                      type="button"
                      onClick={applyCoupon}
                      className="h-14 rounded-2xl bg-[#2b8c7c] px-4 text-[18px] font-medium text-white transition hover:bg-[#35a08f]"
                    >
                      Apply
                    </button>
                  </div>
                ...
                <div className="rounded-2xl bg-white/[0.05] p-5 text-white/70 shadow-[inset_0_0_0_1px_rgba(255,255,255,0.07)]">
                  Your cart is empty. Go back to pricing and add a plan first.
                </div>
              )}
            </div>

            <section className="mt-7 rounded-[24px] bg-[rgba(14,16,19,0.92)] p-5 shadow-[0_30px_80px_rgba(0,0,0,0.34)]">
              ...
              <div ref={guildBoxRef} className="relative">
                <button
                  ...
                  className="flex h-[74px] w-full items-center justify-between rounded-[20px] bg-[#11141a] px-5 text-left transition shadow-[inset_0_0_0_1px_rgba(255,255,255,0.07)] hover:shadow-[inset_0_0_0_1px_rgba(255,255,255,0.09)]"
                >
                  <div className="flex min-w-0 items-center gap-4">
                    <div className="flex h-11 w-11 items-center justify-center rounded-2xl bg-white/[0.06] shadow-[inset_0_0_0_1px_rgba(255,255,255,0.07)]">
                      <ServerGlyph />
                    </div>
                    <div className="min-w-0">
                      {selectedGuild ? (
                        ...
                        <div className="text-[18px] text-white/58">
                ...
                {guildOpen ? (
                  <div className="absolute z-40 mt-3 w-full overflow-hidden rounded-[22px] bg-[#0a0d12]/98 shadow-[0_25px_90px_rgba(0,0,0,0.58),inset_0_0_0_1px_rgba(255,255,255,0.07)] backdrop-blur-2xl">
                    <div className="p-3">
                      <input
                        value={guildFilter}
                        onChange={(e) => setGuildFilter(e.target.value)}
                        placeholder="Search your servers"
                        className="h-12 w-full rounded-2xl bg-white/[0.06] px-4 text-[15px] text-white outline-none placeholder:text-white/35 shadow-[inset_0_0_0_1px_rgba(255,255,255,0.07)] focus:shadow-[inset_0_0_0_1px_rgba(167,139,250,0.45)]"
                      />
                    </div>
                            ...
                            <button
                              ...
                              className={`flex w-full items-center gap-4 rounded-2xl px-3 py-3 text-left transition ${
                                selectedGuildId === guild.id
                                  ? 'bg-white/[0.08] shadow-[inset_0_0_0_1px_rgba(255,255,255,0.07)]'
                                  : 'hover:bg-white/[0.06]'
                              }`}
                            >
            ...
            <section className="mt-7 rounded-[24px] bg-[rgba(14,16,19,0.92)] p-5 shadow-[0_30px_80px_rgba(0,0,0,0.34)]">
              <div className="flex items-center justify-between gap-4">
                ...
                <button
                  ...
                  className="inline-flex items-center gap-2 rounded-2xl bg-white/[0.05] px-4 py-3 text-[16px] font-medium text-white/88 transition hover:bg-white/[0.06] shadow-[inset_0_0_0_1px_rgba(255,255,255,0.07)]"
                >
                  <span className="text-xl leading-none">+</span>
              ...
              <div className="mt-4 space-y-3">
                {selectedAddress ? (
                  <button
                    type="button"
                    onClick={() => setAddressOpen(true)}
                    className="w-full rounded-[20px] bg-white/[0.05] p-5 text-left transition hover:bg-white/[0.05] shadow-[inset_0_0_0_1px_rgba(255,255,255,0.07)]"
                  >
                    <div className="text-[18px] font-medium text-white">{selectedAddress.fullName}</div>
                    <div className="mt-2 text-[15px] leading-7 text-white/62">
                  ...
                  <button
                    type="button"
                    onClick={() => setAddressOpen(true)}
                    className="w-full rounded-[20px] bg-white/[0.06] p-5 text-left text-white/55 transition hover:bg-white/[0.06] shadow-[inset_0_0_0_1px_rgba(255,255,255,0.05)]"
                  >
                  ...
                  <button
                    ...
                    className={`rounded-[20px] p-4 text-left transition ${
                      selectedPayment === method.id
                        ...
                        : 'bg-white/[0.05] hover:bg-white/[0.05] shadow-[inset_0_0_0_1px_rgba(255,255,255,0.07)]'
                    }`}
                  >
                    <div className="flex items-center gap-3">
                      <div className="flex h-11 w-11 items-center justify-center rounded-2xl bg-black/20 shadow-[inset_0_0_0_1px_rgba(255,255,255,0.07)]">
                        <img src={method.icon} alt={method.label} className="max-h-5 opacity-95" />
                      </div>
                      <div>
                        <div className="text-[18px] font-medium text-white">{method.label}</div>
            ...
            <section className="mt-7 rounded-[24px] bg-[rgba(14,16,19,0.92)] p-5 shadow-[0_30px_80px_rgba(0,0,0,0.34)]">
              <label className="flex items-start gap-3 text-[15px] text-white/68">
                <input
                  type="checkbox"
              ...
              <button
                ...
                className="mt-5 h-14 w-full rounded-[18px] bg-[linear-gradient(180deg,#4ca593_0%,#347b6d_100%)] text-[18px] font-medium text-white shadow-[0_18px_40px_rgba(52,123,109,0.3)] transition hover:brightness-110 disabled:cursor-not-allowed disabled:opacity-50"
              >
      ...
      {addressOpen ? (
        <div className="fixed inset-0 z-[90] flex items-center justify-center bg-black/70 px-4 backdrop-blur-md">
          <div className="w-full max-w-[480px] rounded-[28px] bg-[#13161b]/95 p-6 shadow-[0_40px_120px_rgba(0,0,0,0.55),inset_0_0_0_1px_rgba(255,255,255,0.07)]">
            <div className="flex items-center justify-between">
              <div>
                <div className="text-[24px] font-semibold text-white">Your details</div>
                <div className="mt-1 text-sm text-white/55">We collect this information to help combat fraud, and to keep your payment secure.</div>
              </div>
            ...
            <div className="mt-6 space-y-4">
              <Field label="Email address" required>
                <input value="" readOnly className="h-12 w-full rounded-2xl bg-white/[0.05] px-4 text-white outline-none shadow-[inset_0_0_0_1px_rgba(96,188,255,0.78)]" />
              </Field>
              ...
              <Field label="Full name" required>
                <input
                  ...
                  className="h-12 w-full rounded-2xl bg-white/[0.05] px-4 text-white outline-none transition shadow-[inset_0_0_0_1px_rgba(255,255,255,0.07)] focus:shadow-[inset_0_0_0_1px_rgba(255,255,255,0.16)]"
                />
              </Field>
            ...
            <button
              type="button"
              onClick={saveAddress}
              className="mt-6 h-12 w-full rounded-2xl bg-[#4a8b7d] text-[17px] font-medium text-white transition hover:bg-[#57a18f]"
            >
              Continue
            </button>
          </div>
