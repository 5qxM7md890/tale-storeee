          ...
          <tbody>
            {invoices.map((inv) => (
              <tr key={inv.id} className="border-t border-white/10 align-top">
                <td className="px-4 py-3 text-white/80 break-all">{inv.stripeInvoiceId}</td>
                <td className="px-4 py-3 whitespace-nowrap text-white/80">${(inv.amountPaid / 100).toFixed(2)}</td>
                <td className="px-4 py-3">
                  <span className={`inline-flex items-center rounded-full border px-2 py-1 text-xs ${statusClasses(inv.status)}`}>
                    {inv.status}
                  </span>
                </td>
                <td className="px-4 py-3 whitespace-nowrap text-white/70">{new Date(inv.invoiceCreated).toLocaleDateString()}</td>
                <td className="px-4 py-3">
                  {inv.hostedInvoiceUrl ? (
                    <a
                      className="inline-flex rounded-xl border border-white/10 bg-white/5 px-3 py-2 text-xs font-semibold hover:bg-white/10"
