...
export default async function SubscriptionsPage({
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-6 px-4 py-10 sm:px-6 md:py-14">
      <div>
        <h1 className="text-3xl font-extrabold tracking-tight break-words">Your Subscriptions</h1>
        {searchParams?.err === 'no_customer' ? (
          <div className="mt-4 rounded-2xl border border-amber-400/20 bg-amber-400/10 p-4 text-sm text-amber-200">
            No billing customer record was found yet. Please subscribe first, then use Manage billing.
          </div>
        ) : null}
      ...
      <div className="grid gap-4 md:grid-cols-3">
        ...
        <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
          ...
          <div className="mt-1 break-all text-2xl font-bold">{activeGuildId ? activeGuildId : 'Not selected'}</div>
        </div>
        <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
      ...
      <div className="flex flex-col gap-2 sm:flex-row sm:flex-wrap">
        <a
          className="rounded-xl bg-white px-4 py-2 text-center text-sm font-semibold text-black hover:opacity-90"
          href={`/${locale}/pricing`}
        >
          New Subscription
        </a>
        <form action="/api/billing/portal" method="post">
          ...
          <button className="rounded-xl border border-white/15 bg-white/5 px-4 py-2 text-center text-sm font-semibold hover:bg-white/10">
            Manage billing
          </button>
        </form>
        <a
          className="rounded-xl border border-white/15 bg-white/5 px-4 py-2 text-center text-sm font-semibold hover:bg-white/10"
      ...
      <div className="space-y-3">
        {items.length === 0 ? (
          ...
          items.map((it) => {
            ...
            return (
              <div key={it.id} className="rounded-2xl border border-white/10 bg-white/5 p-4 sm:p-5">
